...
div
  div.text-center.position-relative
    button.score__help-btn(
      id="scoreHelp"
      type="button"
      :aria-label="$t('scoreView.helpAriaLabel')"
    )
      icon(name="question-circle" scale="0.9")
    b-tooltip(target="scoreHelp" placement="left" triggers="hover focus")
      | {{ $t('scoreView.tooltipPrefix') }} #[b {{ $t('settings.title') }} &gt; {{ $t('settings.categorization.categories') }}]. {{ $t('scoreView.tooltipSuffix') }}
    div.small.text-muted {{ $t('scoreView.scoreForPeriod') }} {{ score_period_label }}
    ...
    div.small.text-muted(v-if="!isNaN(score_productive_percent)")
      | {{score_productive_percent.toFixed(1)}}% {{ $t('scoreView.productiveSuffix') }}
  hr
  div
    b {{ $t('scoreView.topProductive') }}
    ...
    p.text-muted.small.mb-0.mt-2(v-if="top_productive.length === 0")
      | {{ $t('scoreView.noProductiveCategories') }}
  hr
  div
    b {{ $t('scoreView.topDistracting') }}
    ...
    p.text-muted.small.mb-0.mt-2(v-if="top_distracting.length === 0")
      | {{ $t('scoreView.noDistractingActivity') }}
</template>
...
export default {
  ...
  computed: {
    ...
    score_period_label: function (): string {
      ...
      return timeperiod ? periodReadable(timeperiod) : this.$t('scoreView.selectedPeriod');
    },
