<template lang="pug">
  div
    b-alert(v-if="isVisible", variant="info", show)
      | {{ $t('newReleaseNotification.newReleaseAvailable', { version: latestVersion }) }}
      | #[a(href="https://activitywatch.net/downloads/" target="_blank" class="alert-link") {{ $t('newReleaseNotification.download') }}],
      | {{ $t('newReleaseNotification.youCanAlso') }} #[a(href="javascript:void(0);" class="alert-link" @click="disableCheck") {{ $t('newReleaseNotification.disable') }}]
      | {{ $t('newReleaseNotification.futureReminders') }}
      button(type="button", class="close", @click="isVisible=false") &times;

    b-alert(v-if="isFollowUpVisible", variant="success", show)
      | {{ $t('newReleaseNotification.disabledFollowUp') }}
      | #[router-link(to="/settings" class="alert-link" @click.native="isFollowUpVisible=false") {{ $t('newReleaseNotification.settingsPage') }}].
