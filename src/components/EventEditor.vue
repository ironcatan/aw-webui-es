<template lang="pug">
b-modal(v-if="event && event.id", :id="'edit-modal-' + event.id", ref="eventEditModal", :title="$t('eventEditor.title')", centered, hide-footer)
  ...
  div(v-else)
    table(style="width: 100%")
      tr
        th {{ $t('bucketValidate.bucketLabel') }}
        td {{ bucket_id }}
      tr
        th {{ $t('eventEditor.idLabel') }}
        td {{ event.id }}
      tr
        th {{ $t('query.start') }}
      ...
      tr
        th {{ $t('categoryBuilder.durationCol') }}
    ...
    table(style="width: 100%")
      tr
        th {{ $t('eventEditor.keyLabel') }}
        th {{ $t('eventEditor.valueLabel') }}
