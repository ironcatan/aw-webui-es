...
div
  ...
  div.d-flex.mt-1
    span.mr-auto.small.text-muted {{ $t('report.hostnameLabel') }} {{queryOptions.hostname}}
    b-button.border-0(size="sm", variant="outline-dark" @click="show_options = !show_options")
      span(v-if="!show_options")
        | #[icon(name="angle-double-down")] {{ $t('categoryBuilder.showOptions') }}
      span(v-else)
        | #[icon(name="angle-double-up")] {{ $t('categoryBuilder.hideOptions') }}
  ...
  div(v-if="events != null")
    ...
    div.d-flex
      div.flex-fill
        | {{ $t('report.foundEvents', { count: events.length, seconds: (queryTime / 1000).toFixed(2) }) }}
      div
    ...
    div.small.text-muted(v-if="events.length > 500")
      | {{ $t('report.showingRecent', { count: 500, total: events.length }) }}
